...
void XW(int in_dim, int out_dim, float *in_X, float *out_X, float *W)
{
  ...
  #pragma omp parallel
  {
    ...
    for (int i_block = 0; i_block < v_num; i_block += block_size)
    {
      for (int j_block = 0; j_block < out_dim; j_block += block_size)
      {
        for (int k_block = 0; k_block < in_dim; k_block += block_size)
        {
          for (int i = i_block; i < min(i_block + block_size, v_num); i++)
          {
            for (int j = j_block; j < min(j_block + block_size, out_dim); j++)
            {
              for (int k = k_block; k < min(k_block + block_size, in_dim); k++)
              {
